...
即使官方分支强制改写了提交历史，脚本也会将干净的部署源码同步到最新官方版本。若源码中存在未提交的已跟踪文件修改，更新会停止并提示处理，不会直接覆盖这些修改。
...
真实源码全功能测试会启动临时 Docker 容器，使用真实 `git`、真实 `pnpm` 拉取并构建官方前后端源码，覆盖安装、更新、配置、查看、备份、恢复、清理、WebDAV、卸载，并启动后端 bundle 检查监听和 HTTP 响应。当前测试环境没有真实 systemd，服务控制会验证 `systemctl` 调用分支；默认测试镜像是 `node:22-bookworm`，也可以用 `SUB_STORE_REAL_TEST_IMAGE=镜像名` 指定。
